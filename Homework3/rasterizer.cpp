...
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos) {
    ...
    int min_x = floor(std::min({v[0].x(), v[1].x(), v[2].x()}));
    int max_x = ceil(std::max({v[0].x(), v[1].x(), v[2].x()}));
    int min_y = floor(std::min({v[0].y(), v[1].y(), v[2].y()}));
    int max_y = ceil(std::max({v[0].y(), v[1].y(), v[2].y()}));
